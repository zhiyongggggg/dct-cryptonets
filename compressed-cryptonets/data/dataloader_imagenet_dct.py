...
import time
import torch
from .dataset_imagenet_dct import ImageFolderDCT, ImageFolder
# from . import cvtransforms_DCTNet as transforms
from . import cvtransforms as transforms
from data import train_y_mean, train_y_std, train_cb_mean, train_cb_std, \
    train_cr_mean, train_cr_std
from data import train_y_mean_upscaled, train_y_std_upscaled, train_cb_mean_upscaled, train_cb_std_upscaled, \
...
from data import train_upscaled_static_dct_direct_mean_interp, train_upscaled_static_dct_direct_std_interp
...
def trainloader_upscaled_dct_direct(args, model='mobilenet'):
    ...
    # transform = transforms.Compose([
    #     transforms.RandomResizedCrop(filter_size * 56),
    #     transforms.ImageJitter(self.jitter_param),
    #     transforms.RandomHorizontalFlip(),
    #     transforms.GetDCT(filter_size),
    #     transforms.UpScaleDCT(size=56),
    #     transforms.ToTensorDCT(),
    #     transforms.SubsetDCT(channels=6),
...
def trainloader_nondct(args, model='mobilenet'):
    traindir = os.path.join(args.data, 'train')
    ...
    train_dataset = ImageFolder(traindir, transform)
...
def valloader_nondct(args, model='mobilenet'):
    valdir = os.path.join(args.data, 'val')
    ...
    val_dataset = ImageFolder(valdir, transform)
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset,
        batch_size=args.test_batch, shuffle=(val_sampler is None),
        num_workers=args.workers, pin_memory=True, sampler=val_sampler)

    val_loader_len = len(val_loader)

    return val_loader, val_sampler, val_loader_len
