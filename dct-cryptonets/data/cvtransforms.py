...
class SubsetDCT(object):
    def __init__(self, channels=24, pattern='default', filter_size=8):
        if filter_size == 4:
            self.subset_channel_index = subset_channel_index_filtersize_4
        ...
        if channels < 192:
            ...
            self.subset_y = list(range(0, 64))
            self.subset_cb = list(range(0, 64))
            self.subset_cr = list(range(0, 64))
...
class NormalizeDCT(object):
    ...
    """
    def __init__(self, y_mean, y_std, cb_mean=None, cb_std=None, cr_mean=None, cr_std=None, channels=None, pattern=None):
...
class Rescale(object):
    """ Apply tensor rescaling to [0:1] based on min and max values """
    def __call__(self, tensor):
        if type(tensor) is tuple:
            tensor_min, _ = torch.min(tensor[0], dim=1, keepdim=True)
            tensor_max, _ = torch.max(tensor[0], dim=1, keepdim=True)
            rescaled_tensor = (tensor[0] - tensor_min) / (tensor_max - tensor_min)
            rescaled_tensor = torch.nan_to_num(rescaled_tensor)
        else:
            tensor_min, _ = torch.min(tensor, dim=1, keepdim=True)
            tensor_max, _ = torch.max(tensor, dim=1, keepdim=True)
            rescaled_tensor = (tensor - tensor_min) / (tensor_max - tensor_min)
            rescaled_tensor = torch.nan_to_num(rescaled_tensor)
        return rescaled_tensor


subset_channel_index_filtersize_4 = {
    24:
    [
        [0, 1, 2, 3,
         4, 5, 6, 7,
